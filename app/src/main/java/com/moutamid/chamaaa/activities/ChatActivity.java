...
import com.moutamid.chamaaa.adapters.ChatAdapter;
import com.moutamid.chamaaa.adapters.ContributionAdapter;
import com.moutamid.chamaaa.bottomsheets.ChatMenu;
import com.moutamid.chamaaa.bottomsheets.ShowProducts;
import com.moutamid.chamaaa.bottomsheets.WithdrawFunds;
import com.moutamid.chamaaa.databinding.ActivityChatBinding;
import com.moutamid.chamaaa.listener.ChatMenuListener;
...
import com.moutamid.chamaaa.notifications.NotificationScheduler;
                    ...
                    Constants.databaseReference().child(Constants.CHATS).child(stashUser.id).child(chatModel.id).updateChildren(map)
                            .addOnSuccessListener(unused1 -> {
                                if (chatModel.isGroup) {
                                    ...
                                    new FCMNotificationHelper(this, chatModel.id, false).sendNotification(id, chatModel.name, m);
                                } else {
                                    Constants.databaseReference().child(Constants.CHATS).child(chatModel.userID).child(chatModel.id).updateChildren(map)
                                            .addOnSuccessListener(unused2 -> {
                                                ...
                                                new FCMNotificationHelper(this, chatModel.id, false).sendNotification(id, chatModel.name, map.get("lastMessage").toString());
                                            });
                                }
                            });
                });
