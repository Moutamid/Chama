...
import com.moutamid.chamaaa.databinding.CreatePollBinding;
...
import com.moutamid.chamaaa.models.Votes;
                            ...
                            Constants.databaseReference().child(Constants.CHATS).child(stashUser.id).child(chatModel.id).updateChildren(map)
                                    .addOnSuccessListener(unused1 -> {
                                        if (chatModel.isGroup) {
                                            ...
                                            new FCMNotificationHelper(requireContext(), chatModel.id, false).sendNotification(id, chatModel.name, m);
                                        }
                                    });
                        });
            }
