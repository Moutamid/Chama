...
import com.moutamid.chamaaa.adapters.PersonAdapter;
import com.moutamid.chamaaa.databinding.SendMoneyBinding;
        ...
        Constants.databaseReference().child(Constants.CHATS).child(userModel.id).child(chatModel.id).updateChildren(map)
                .addOnSuccessListener(unused1 -> {
                    ...
                    if (chatModel.isGroup) {
                        ...
                        new FCMNotificationHelper(requireContext(), chatModel.id, false).sendNotification(id, chatModel.name, map.get("whoShared").toString());
                    } else {
